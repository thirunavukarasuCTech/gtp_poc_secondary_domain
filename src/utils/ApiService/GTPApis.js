...
export const getUserAuthStatusForSocialLogin = async (user_id) => {
  const url = `https://fortress-backend-315ca6662153.herokuapp.com/getUserAuthStatus/${user_id}`;
  // const url = 'https://fortress-backend-315ca6662153.herokuapp.com/check-auth';
  const response = await axios.get(url);
  return response;
}
export const getSocialAuthTokens = async (code) => {
  try {
    const url = 'https://fortress-backend-315ca6662153.herokuapp.com/api/getAuthTokens'
    const payload = {
      code
    };
...
export const auth0Logout = async (returnTo) => {
  const url = 'https://fortress-backend-315ca6662153.herokuapp.com/api/logout';
  const response = await axios.get(url);
...
export const signupAndAuthorize = (signupInfo, cardOrNewsLetterInformation, callback) => {
  let username;
  if (cardOrNewsLetterInformation && cardOrNewsLetterInformation.isCardDataAvailable) {
    username = cardOrNewsLetterInformation.cardData.cardNumber;
  }
  let signupObject = {
    connection: 'Username-Password-Authentication',
    email: signupInfo.email,
    password: signupInfo.password,
    username: username ? username : signupInfo.mobile,
    scope: 'openid',
  }
